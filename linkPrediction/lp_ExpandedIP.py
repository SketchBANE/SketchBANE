...
def linkPredicton(dataset,T,K):
    ...
    embedding = np.hstack((embedding, new_embedding))

    if dataset == 'Amazon' or dataset == 'ogbn-papers100M':
        file_name2 = "../datas/" + dataset + "/lp/" + dataset + "_trainGraph_" + str(ratio) + ".npz"
        file_name3 = "../datas/" + dataset + "/lp/" + dataset + "_testGraph_" + str(ratio) + ".npz"
        trainGraph = load_npz(file_name2)
        testGraph = load_npz(file_name3)
    else:
        file_name2 = "../datas/" + dataset + "/lp/" + dataset + "_" + str(ratio) + ".mat"
    ...
    for i in range(5):
        ...
        nonexistence_similarity = np.diag(
            np.dot(embedding[nonexistence[0], :], embedding[nonexistence[1], :].T))
        ...
        missing_similarity = np.diag(
            np.dot(embedding[testedEdges[:, 0], :], embedding[testedEdges[:, 1], :].T))
    ...
    log_path = '../results/' + dataset + '/lp_ExpandedIP.log'
